...
bool contains(chunk_t *pointer_array[], void *pointer)
{
  ...
  {
      if(pointer_array[index]->key == pointer)
        return true;
    index++;
 }
  return false;
...
Test(asserts, allocation_returns_pointer)
{
 ...
 Test(asserts, allocation_has_correct_position_in_chunk_buffer)
 {
 
   void *pointer = my_malloc(10);
   void *pointer_two = my_malloc(10);
   void *pointer_three = my_malloc(10);
 
   chunk_t *chunk = find_alloc(pointer);
   chunk_t *chunk_two = find_alloc(pointer_two);
   chunk_t *chunk_three = find_alloc(pointer_three);
 
   cr_assert(chunk->location == 0, "Pointer one: Chunk buffer position is incorrect\n");
   cr_assert(chunk_two->location == 1, "Pointer two: Chunk buffer position is incorrect\n");
   cr_assert(chunk_three->location == 2, "Pointer three: Chunk buffer position is incorrect\n");
